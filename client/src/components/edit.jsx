...
export default () => {
    ...
    const [game, setThisGame] = useState("");


    useEffect(() => {
        axios.get("http://localhost:7000/api/games/" + id)
            .then(res => {
                console.log(res.data);
                setThisGame(res.data);
    ...
    const onSubmitHandler = e => {
        e.preventDefault();
        axios.put('http://localhost:7000/api/games/' + id, { title, genre, url, image })
            .then(res => {
                console.log(res.data);
    ...
    return (
        ...
        < form onSubmit={onSubmitHandler} >
            <VFXDiv shader='rgbGlitch' className='genre' >{game.title} </VFXDiv>
